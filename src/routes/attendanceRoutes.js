...
router.post('/selfie-mark', protect, authorize('EMPLOYEE', 'ADMIN'), markSelfieAttendance);
router.get('/selfie/:id/:type', protect, getSelfieImage);
router.get('/geofence-config', protect, getGeofenceConfig);
router.put('/geofence-config', protect, authorize('ADMIN'), updateGeofenceConfig);
router.patch('/review/:id', protect, authorize('ADMIN'), reviewAttendanceRecord);
...
router.get('/today', protect, authorize('EMPLOYEE', 'ADMIN'), getTodayAttendance);
...
router.get('/monthly', protect, authorize('EMPLOYEE', 'ADMIN'), getMonthlyStats);

// ── ADMIN-ONLY ROUTES ──────────────────────────────────────────────────────
router.post('/generate-qr', protect, authorize('ADMIN'), generateQr);
router.get('/admin/stats', protect, authorize('ADMIN'), getAdminStats);
router.get('/admin/daily-list', protect, authorize('ADMIN'), getDailyAttendanceList);
